...
function ContractAutomationSolution() {
  ...
  useEffect(() => {
    setHeaderData({ title: Helpers.getTranslationValue('contract_automation_solution'), desc: '' });
  }, [setHeaderData]);
  ...
  const handleFileUpload = async () => {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      try {
        const response = await axios.post(`${Helpers.apiUrl}contract-automation`, formData, Helpers.authFileHeaders);
  ...
  return (
    <div className="w-full bg-white py-5 mx-auto">
      <h2 className="text-center text-2xl font-semibold mb-8">{Helpers.getTranslationValue('contract_automation_solution')}</h2>
